...
describe('getAccidentals', () => {
    ...
    for (const index in modes) {
        it('should return correct accidentals for ' + modes[index][0], () => {
            const result = Key.getAccidentals(modes[index][0], 'g');
...
describe('toggleAccidental', () => {
    const clefs = ['g', 'f'];
    const clefAccidentalSharpPositions = {
        g: [0, 3, -1, 2, 5, 1, 4],
        f: [2, 5, 1, 4, 7, 3, 6],
    };
    for (const clef of clefs) {
        it('should set key if custom accidentals are valid for key (single accidental, ' + clef + ')', () => {
            // NOTE: actual key (here f) is not important for this test
            const key = new Key('c', 'major');
            const result = key.toggleAccidental(0, clefAccidentalSharpPositions[clef][0], '#', 'major', clef);
            expect(result).toBeTruthy();
            expect(key.accidentals.count).toBe(1);
            expect(key.mode).toBe('major');
            expect(key.root).toBe('g');
        });
        it('should set key if custom accidentals are valid for key (two accidentals, ' + clef + ')', () => {
            // NOTE: actual key (here f) is not important for this test
            const key = new Key('f', 'major');
        ...
        it('should remove last accidental if same as before (' + clef + ')', () => {
            ...
            expect(key.getCustomAccidentals(clef)[0].position).toBe(clefAccidentalSharpPositions[clef][0] + 1);
        });
