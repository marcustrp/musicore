...
export class RhythmElement {
    ...
    getChordSymbol(index = 0) {
        if (!this._chordSymbols || this._chordSymbols.length <= index)
            return;
     ...
     * @returns
     */
    getChordSymbolByPosition(offset, level) {
    ...
    getChordSymbolIndexByPosition(offset, level) {
        if (!this._chordSymbols)
            return;
        // both offset and level are undefined instead of 0
        if (offset && offset.equals(0))
            offset = undefined;
        if (level === 0)
            level = undefined;
        return this._chordSymbols.findIndex((chordSymbol) => this.findChordSymbol(chordSymbol, offset, level));
    }
    findChordSymbol(chordSymbol, offset, level) {
        if (offset &&
    ...
    getDuration(ignoreTriplet = false) {
        const fraction = this.triplet && !ignoreTriplet ?
     ...
     */
    setTriplet(numerator, denominator, totalDuration, noteIndex, noteCount) {
        this.triplet = {
            numerator: numerator,
        ...
        if (noteIndex === 0)
            this.triplet.start = true;
        if (noteIndex === noteCount - 1)
            this.triplet.end = true;
    }
    /**
     *
     * @param items
     ...
     * @todo add error handling
     */
    static calculateTripletDuration(items, numerator, denominator) {
