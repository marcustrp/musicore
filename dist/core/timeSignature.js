...
export class TimeSignature {
     ...
     */
    fillBeamGroupDurations(durations) {
        ...
        if (totalDuration.compare(this.duration) > 0) {
            throw new Error('The total duration of the beam group durations is greater than the timeSignature duration');
        }
     ...
     */
    getNotesFromBeamGroup(startDuration, note, overflow = 'process') {
        if (overflow === 'return')
        ...
        while (noteDurationLeft.compare(0) > 0 &&
            ...
            if (groupDurationLeft.compare(noteDurationLeft) > 0) {
                ...
                data[key].push(clone);
                addedBeamDuration = new Fraction(this.beamGroupDurations[beamGroupIndex % this.beamGroupDurations.length]);
            }
            else if (groupDurationLeft.compare(this.beamGroupDurations[beamGroupIndex % this.beamGroupDurations.length]) === 0 &&
                this.beamGroupIndexesInSameBar(beamGroupIndex, beamGroupIndex + 1) &&
                noteDurationLeft.compare(groupDurationLeft.add(this.beamGroupDurations[(beamGroupIndex + 1) % this.beamGroupDurations.length])) >= 0) {
                console.log('note is at start of beam group and at least as long as two beam groups, which are in the same bar');
                // note is at start of beam group and at least as long as two beam groups, which are in the same bar
                //let span = 2;
                beamGroupIndex++;
                let tmpGroupDuration = this.beamGroupDurations[beamGroupIndex % this.beamGroupDurations.length].add(this.beamGroupDurations[(beamGroupIndex + 1) % this.beamGroupDurations.length]);
                while (this.beamGroupIndexesInSameBar(beamGroupIndex, beamGroupIndex + 1) &&
                    groupDurationLeft
                        .add(this.beamGroupDurations[(beamGroupIndex + 1) % this.beamGroupDurations.length])
                        .compare(noteDurationLeft) > 0) {
                    beamGroupIndex++;
                    tmpGroupDuration = tmpGroupDuration.add(this.beamGroupDurations[beamGroupIndex % this.beamGroupDurations.length]);
                    //span++;
                }
                ...
                console.log('other cases', groupDurationLeft.toString(), noteDurationLeft.toString(), groupDurationLeft.compare(this.beamGroupDurations[beamGroupIndex % this.beamGroupDurations.length]), this.beamGroupIndexesInSameBar(beamGroupIndex, beamGroupIndex + 1), beamGroupIndex, noteDurationLeft.compare(groupDurationLeft.add(this.beamGroupDurations[(beamGroupIndex + 1) % this.beamGroupDurations.length])));
      ...
      data.overflow.push(clone);
        ...
        if (overflow === 'process' && data.overflow.length > 0 && data.notes[0] instanceof Note) {
            data.notes[data.notes.length - 1].tie =
                data.notes.length > 1 ? 'continue' : 'start';
            data.overflow[0].tie = data.overflow.length > 1 ? 'continue' : 'end';
        }
        return data;
    }
    /**
    ...
    beamGroupIndexesInSameBar(index1, index2) {
        return (Math.floor(index1 / this.beamGroupDurations.length) ===
