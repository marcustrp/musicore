...
export class Scale {
     ...
     */
    constructor(root, type, customNoteNames) {
        this.type = type;
        ...
        if (type === 'custom') {
            if (!customNoteNames)
                throw new Error('No customNoteNames provided for custom scale.');
            ...
            customNoteNames[0].length > 1 ?
                customNoteNames[0].substring(1)
                : undefined;
            // TODO: loop customNoteNames from provided root, calling getScaleNumberFromNote...
            customNoteNames.forEach((notename) => {
                this.scaleNumbers.push(major.getScaleNumberFromNote(notename.substring(0, 1), notename.length > 1 ? notename.substring(1) : undefined));
                const pitchDiff = Note.pitchDiff(notename, customNoteNames[0]);
                if (pitchDiff === undefined)
                    throw new Error('Invalid customNotes');
     ...
     */
    static fromString(rootAndMode) {
        if (!rootAndMode)
            throw new Error('Empty rootAndMode');
        const data = rootAndMode.split(' ');
        if (data.length !== 2)
            throw new Error('Invalid rootAndMode');
     ...
     */
    getScaleNumberIndex(midiNumber) {
        if (midiNumber >= 0 && midiNumber < 128)
            return (midiNumber - (this.root.number % 12)) % 12;
        else
            return -1;
    }
    /**
    ...
    getDiatonicNoteNames() {
        return this.scaleNumbers.map((scaleNumber) => Note.noteNumberToNameInScale(this.rootName, scaleNumber));
    }
    /**
    ...
    static validateScaleType(type) {
        return type in scaleTypes;
    }
    static modeFromNotes(notes, requireDescending = true) {
        const rootScale = new Scale(notes[0].name, 'major');
        const scaleNumbers = notes.map((note) => rootScale.getScaleNumberFromNote(note.root, note.accidental));
        const mode = Scale.modeFromScaleNumbers(scaleNumbers, requireDescending);
        return mode;
    }
    static modeFromScaleNumbers(scaleNumbers, requireDescending = true) {
        let mode = undefined;
    ...
    static validateScaleNumbersToMode(modeScaleNumbers, scaleNumbers) {
        let isValid = true;
        modeScaleNumbers.forEach((scaleNumber, i) => (isValid &&= scaleNumber === scaleNumbers[i]));
        return isValid;
    }
    static parseScaleNumberArray(scaleNumbers) {
        const ascending = [];
        const descending = [];
     ...
     * @returns
     */
    getModeDefaultScaleNumber(scaleNumber) {
        const modeScaleNumber = this.scaleNumbers.find((item) => item.indexOf(scaleNumber.toString()) >= 0);
        if (modeScaleNumber) {
            return modeScaleNumber;
        }
        else {
            return scaleNumber.toString();
        }
    }
    /**
     ...
     */
    getScaleNumberFromNote(root, accidental, relativeToMajor = true) {
        const scale = relativeToMajor ?
     ...
     * @returns
     */
    getNotePitchDiffFromScale(root, accidental) {
        ...
        const diff = Note.nameToNoteIndex(root + (accidental || '')) -
            Note.nameToNoteIndex(noteNames[noteNumber - 1]);
        return (diff > 6 ? diff - 12
            : diff < -6 ? diff + 12
                : diff);
    }
    /**
     ...
     * @returns 1-7
     */
    getDiatonicScaleNumberFromNote(root, scale) {
        ...
        return (noteNames.findIndex((item) => item.charAt(0) === root) + 1);
    }
