...
describe('Note.processScaleNumber', () => {
    ...
    it('should return correct data for 1 in c major', () => {
        ...
        const scaleNumberParts = { number: 1, accidental: undefined };
    ...
    it('should return correct data for 3 in c minor (not using scale mode)', () => {
        ...
        const scaleNumberParts = { number: 3 };
    ...
    it('should return correct data for m3 in c minor (using scale mode)', () => {
        const scale = new Scale('c', 'minor');
        const scaleNumberParts = { number: 3, accidental: 'm' };
        const result = Note['processScaleNumber'](scaleNumberParts, scale, true);
        expect(result).toEqual({ number: 3, accidental: undefined });
    });
});
describe('Note.fromScaleNumber', () => {
    ...
    it('should create a simple note', () => {
        ...
        const noteIndex = 6;
        const noteNumberParts = { number: 3, accidental: undefined };
        const noteObj = {
            type: 'q',
            root: 'f',
            accidental: '#',
        ...
        vi.spyOn(Note, 'noteNumberToNameInScale').mockImplementationOnce(() => noteObj.root + (noteObj.accidental || ''));
        const result = Note.fromScaleNumber(((noteNumberParts.accidental || '') + noteNumberParts.number), 4, scaleMock.get(scaleData));
        expect(result).toMatchObject(noteObj);
    });
    it('should create a note using scaleNumber with accidental', () => {
        const scaleData = { rootNumber: 2, rootName: 'd' };
        const noteIndex = 8;
        const noteNumberParts = { number: 4, accidental: '#' };
        const noteObj = {
            root: 'g',
            accidental: '#',
            octave: 4,
    ...
    it('should create a note using minor scale', () => {
        ...
        const result = Note.fromScaleNumber(((noteNumberParts.accidental || '') + noteNumberParts.number), 5, scaleMock.get(scaleData));
        expect(result).toMatchObject(noteObj);
    });
    it('should create a note using minor scale with m (major) accidental', () => {
        const scaleData = { rootNumber: 2, rootName: 'd' };
        const noteIndex = 6;
        const noteNumberParts = { number: 3, accidental: 'm' };
        const noteObj = {
            root: 'f',
            accidental: '#',
            octave: 5,
    ...
    it('should create a simple note using scaleNumber 9', () => {
        ...
        const noteIndex = 10;
        const noteNumberParts = { number: 9, accidental: undefined };
        const noteObj = {
            type: 'q',
            root: 'a',
            octave: 6,
    ...
    it('should create a note in correct octave', () => {
        ...
        const noteIndex = 4;
        const noteNumberParts = { number: 5, accidental: undefined };
        const noteObj = {
            root: 'e',
            octave: 5,
            midiNumber: 64,
    ...
    edgeCases.forEach((edgeCase) => {
        it('should create a note in correct octave of edge case ' + edgeCase.systemName, () => {
            const noteObj = {
                accidental: edgeCase.systemName.substr(1),
                root: edgeCase.systemName[0],
      ...
      */
            const result = Note.fromScaleNumber(edgeCase.scaleNumber, 5, scaleMock.get({ rootNumber: edgeCase.scaleRootNumber, rootName: edgeCase.scaleRoot }));
            expect(result).toMatchObject(noteObj);
        });
    });
    it('should create a note with correct length', () => {
        const scaleData = { rootNumber: 9, rootName: 'a' };
        const noteIndex = 2;
        const noteNumberParts = { number: 2, accidental: undefined };
        const noteObj = {
            type: 'h',
            dots: 1,
            root: 'b',
        ...
        vi.spyOn(Note, 'getNumberParts').mockImplementationOnce(() => noteNumberParts);
        vi.spyOn(Note, 'noteNumberToNameInScale').mockImplementationOnce(() => noteObj.root);
        const result = Note.fromScaleNumber(((noteNumberParts.accidental || '') + noteNumberParts.number), 4, scaleMock.get(scaleData), 'h', 1);
        expect(result).toMatchObject(noteObj);
    });
    it('should throw on invalid input', () => {
        const resultFn = () => Note.fromScaleNumber('none', 5, new Scale('c', 'major'), 'h');
...
describe('Note.nameToNoteIndex', () => {
    ...
    it('should return -1 for undefined', () => {
        const result = Note.nameToNoteIndex(undefined);
        expect(result).toEqual(-1);
    });
});
...
describe('Note.addNotaion', () => {
    it('should add notation', () => {
        ...
        const note = new Note('q', 'c', undefined, 5);
        note.addNotation({ type: 'text', text: 'test' });
        expect(note).toMatchObject(noteObj);
    });
    it('should throw on undefined input', () => {
        const note = new Note('q', 'c');
        const func = () => note.addNotation(undefined);
        expect(func).toThrow();
    });
});
