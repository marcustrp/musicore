 ...
 */
export const noteEventHandler = (event, dispatchEvent, settings) => {
    if (!settings)
        settings = {};
...
const getNoteAction = (currentNote, newNote, settings) => {
 ...
 * @returns
 */
const getNearestNote = (currentNote, newNote) => {
    let index = 0;
    const newPos = Math.abs(LNoteHead.getPositionFromRoot(newNote.root, newNote.octave, 'g'));
    let distance = Math.abs(LNoteHead.getPositionFromRoot(currentNote.root, currentNote.octave, 'g') - newPos);
...
export const noteAccidentalEventHandler = (event, dispatchEvent) => {
    let note = event.note;
    if (event.index > 0) {
        // index > 0 means chord note[index - 1]
        if (!note.chord || note.chord.length <= event.index - 1)
            throw new Error('Invalid chord index');
        note = note.chord[event.index - 1];
    }
    if ((note.accidental || note.printedAccidental) &&
