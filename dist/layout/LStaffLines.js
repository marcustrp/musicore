...
export class LStaffLines {
    x = 0;
    y = 0;
    bBox = new BBox();
    lines;
    constructor(settings, lineCount, x) {
        this.lines = [];
    ...
    layout(staveSpace, staffMargin, x) {
        this.lines.forEach((staffLine) => {
            //staffLine.y = staffMargin + staveSpace * index;
            staffLine.length = x;
        });
        this.bBox = new BBox(this.lines[0].x, this.lines[0].y - this.lines[0].thickness / 2, this.lines[0].length, this.lines[this.lines.length - 1].y - this.lines[0].y + this.lines[0].thickness);
    }
}
