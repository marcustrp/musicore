...
export class AbcImportState {
    ...
    voiceIndex;
    errors;
    constructor(currentScale = new Scale('c', 'major'), scaleNoteNames = {}, partIndex = 0, voiceIndex = 0, errors = []) {
        this.currentScale = currentScale;
        this.scaleNoteNames = scaleNoteNames;
...
export class AbcImporter {
                ...
                if (line.staff) {
                    line.staff.forEach((staff) => {
                        staff.voices?.forEach((voice, voiceIndex) => {
                            ...
                            if (!staffCreated) {
                                const clefType = staff.clef ? staff.clef.type : 'treble';
                                const clef = new Clef(mappers.clef[clefType].type, undefined, mappers.clef[clefType].octave);
    ...
    initScore(tune) {
        ...
        if (pickupLength > 0)
            this.score.bars.convertToPickup(new Fraction(pickupLength));
        ...
        if (tune.metaText)
            this.metaTextParser.parse(tune.metaText, this.score);
    }
    /** @todo implement more... */
    parseElement(item /*, partIndex: number, staffIndex: number, voiceIndex: number*/) {
        switch (item.el_type) {
            case 'bar':
            ...
            case 'note':
                /** @abcjs incomplete type */
                this.noteParser.parse(item, this.score);
                break;
            case 'overlay':
                break;
            case 'part':
