...
export class MetaTextParser {
    ...
    parse(metaText, score) {
        if (metaText.title)
            score.addWorkInformation('title', metaText.title);
        if (metaText.author)
            score.addWorkInformation('lyrics', metaText.author);
        if (metaText.composer)
            score.addWorkInformation('composer', metaText.composer);
        if (metaText.history)
            score.addWorkInformation('history', metaText.history);
        if (metaText.transcription)
            score.addInformation('transcription', metaText.transcription);
        ...
        if (metaText.book)
            score.addInformation('books', metaText.book);
        //if (metaText.decorationPlacement) score.addInformation(, metaText.decorationPlacement);
        if (metaText.discography)
            score.addInformation('discography', metaText.discography);
        //if (metaText.footer) score.addInformation(, metaText.footer);
        if (metaText.group)
            score.addInformation('group', metaText.group);
        //if (metaText.header) score.addInformation(, metaText.header);
        if (metaText.instruction)
            score.addInformation('instruction', metaText.instruction);
        //if (metaText.measurebox) score.addInformation(, metaText.measurebox);
        if (metaText.notes)
            score.addInformation('notes', metaText.notes);
        if (metaText.origin)
            score.addInformation('origin', metaText.origin);
        ...
        if (metaText.rhythm)
            score.addInformation('type', metaText.rhythm);
        if (metaText.source)
            score.addInformation('source', metaText.source);
        if (metaText.textBlock)
            score.addVerse(metaText.textBlock);
        if (metaText.url)
            score.addInformation('url', metaText.url);
