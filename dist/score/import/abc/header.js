...
export default class HeaderParser {
    ...
    getKey(abcKey) {
        ...
        const mode = abcKey.mode in mappers.keyMode ? mappers.keyMode[abcKey.mode] : 'major';
    ...
    getTimeSignature(abcTimeSignature) {
        switch (abcTimeSignature.type) {
            ...
            case 'specified':
                if (abcTimeSignature.value) {
                    ...
                    return new TimeSignature(+abcTimeSignature.value[0].num, abcTimeSignature.value[0].den ? +abcTimeSignature.value[0].den : undefined);
                }
                else {
