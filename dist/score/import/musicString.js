...
import { BarlineParser } from './musicString/bar.js';
import { BodyParser } from './musicString/body.js';
import { InformationParser } from './musicString/information.js';
import { ModifierParser } from './musicString/modifiers.js';
...
export class MusicStringImporter {
    ...
    parse(musicString) {
        ...
        items.forEach((item, index) => {
            ...
            const bodyItem = bodyParser.parse(item, this.info, this.score.bars.bars[this.score.bars.bars.length - 1]);
            if (bodyItem) {
        ...
        if (bodyParser.slurIndex !== 0)
            this.errors.push('Missing end of slur');
        if (this.errors)
            this.errors.forEach((error) => console.warn('Parser error: ' + error));
        return this.score;
    }
    splitMusicString(musicString) {
        // split by space, but not when space within any type of quote
    ...
    createScore(info) {
        ...
        if (info.tempo)
            this.score.bars.setTempo(info.tempo);
        if (info.title)
            this.score.addWorkInformation('title', info.title);
        if (info.type)
            this.score.addInformation('type', info.type);
        if (info.books)
            info.books.forEach((book) => this.score.addInformation('books', book));
        if (info.discography)
            info.discography.forEach((disc) => this.score.addInformation('discography', disc));
        if (info.notes)
            this.score.addInformation('notes', info.notes);
        if (info.origin)
            this.score.addInformation('origin', info.origin);
        if (info.source)
            this.score.addInformation('source', info.source);
        if (info.transcriptions)
            info.transcriptions.forEach((transcription) => this.score.addInformation('transcription', transcription));
        if (info.composers)
    ...
    handleBarItem(item) {
        ...
        // for current bar
        if (item.barline && item.barline !== 'regular')
            this.score.bars.setBarline(item.barline);
        if (item.repeatEnd && firstBarVoiceDuration.compare(0) != 0)
            this.score.bars.setRepeatEnd(item.repeatEnd);
        if (item.directionsCurrent)
            this.addDirections(item.directionsCurrent);
        if (item.lineBreak)
            this.addLineBreak();
        // for next bar
        if (item.repeatStart || item.ending || item.directionsNext) {
            ...
            if (item.repeatStart)
                this.score.bars.setRepeatStart(item.repeatStart);
            if (item.ending)
                this.score.bars.setEnding(item.ending);
            if (item.directionsNext)
                this.addDirections(item.directionsNext);
        }
    }
