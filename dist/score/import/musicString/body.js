...
export var BodyMatch;
(function (BodyMatch) {
    BodyMatch[BodyMatch["FULL"] = 0] = "FULL";
    BodyMatch[BodyMatch["SLUR_START"] = 1] = "SLUR_START";
    BodyMatch[BodyMatch["TRIPLETS_P"] = 2] = "TRIPLETS_P";
    BodyMatch[BodyMatch["TRIPLETS_Q"] = 3] = "TRIPLETS_Q";
    BodyMatch[BodyMatch["TRIPLETS_R"] = 4] = "TRIPLETS_R";
    BodyMatch[BodyMatch["NOTATION"] = 5] = "NOTATION";
    BodyMatch[BodyMatch["STEP"] = 6] = "STEP";
    BodyMatch[BodyMatch["CHORD_SYMBOL"] = 7] = "CHORD_SYMBOL";
    BodyMatch[BodyMatch["LYRICS"] = 8] = "LYRICS";
    BodyMatch[BodyMatch["SOLFA"] = 9] = "SOLFA";
    BodyMatch[BodyMatch["FUNCTION"] = 10] = "FUNCTION";
    BodyMatch[BodyMatch["GRACE"] = 11] = "GRACE";
    BodyMatch[BodyMatch["LENGTH"] = 12] = "LENGTH";
    BodyMatch[BodyMatch["CHORD_NOTES"] = 13] = "CHORD_NOTES";
    BodyMatch[BodyMatch["LENGTH_2"] = 14] = "LENGTH_2";
    BodyMatch[BodyMatch["CHORD_NOTES_2"] = 15] = "CHORD_NOTES_2";
    BodyMatch[BodyMatch["NOTE"] = 16] = "NOTE";
    BodyMatch[BodyMatch["LENGTH_3"] = 17] = "LENGTH_3";
    BodyMatch[BodyMatch["NOTE_2"] = 18] = "NOTE_2";
    BodyMatch[BodyMatch["DOTS"] = 19] = "DOTS";
    //REST, // rest
    BodyMatch[BodyMatch["TIE"] = 20] = "TIE";
    BodyMatch[BodyMatch["SLUR_END"] = 21] = "SLUR_END";
    /*// (prefix)
  NOTATION = 1,
...
export class BodyParser {
   ...
   */
    match(item, bar) {
        const myRegexp = /([(]*)?(?:([1-9]):(?:([1-9]):)?(?:([1-9]):)?)?(!(?!(?:!|!!)).*!)*(?:`([^"]*)`)?(?:"([^"]*)")?(?:'([^']*)')?(?:´([^']*)´)?(?:\*([^*]*)\*)?(?:<([^>]*)\>)?(?:(__|[ldwhqestuv_](?:{[^}]*})?)|(?:\[((?:(?:[+-]){0,2}(?:(?:bb|b|#|x|m)(?!x))?[1-9rxyi][IL]*(?:{[^}]*})?){0,32})\](__|[ldwhqestuv_](?:{[^}]*})?))|(?:\[((?:(?:[+-/])*(?:(?:bb|b|#|x|m)(?!x))?[1-9rxyi][IL]*(?:{[^}]*})?){0,32})\])|(?:((?:[+-/])*(?:(?:bb|b|#|x|m)(?!x))?[1-9rxyi][IL]*(?:{[^}]*})?)(__|[ldwhqestuv_](?:{[^}]*})?))|((?:[+-/])*(?:(?:bb|b|#|x|m)(?!x))?[1-9rxyi][IL]*(?:{[^}]*})?))([.]*)(=)?([)]*)?/g;
    ...
    matchParse(items, inGroup = false) {
        // get length. If not set, default to q (quarternote)
        const type = this.getNoteType(items, inGroup);
        const typeIsDefault = (items[BodyMatch.LENGTH_3] || items[BodyMatch.LENGTH_2] || items[BodyMatch.LENGTH] || '') ===
            '';
        // of not set default to 1
        //const numberOrChord = items[17] || items[12] || items[7] || items[3] || '1';
        const numberOrChord = items[BodyMatch.CHORD_NOTES_2] ||
            items[BodyMatch.NOTE_2] ||
            items[BodyMatch.CHORD_NOTES] ||
            items[BodyMatch.NOTE] ||
            '1';
        const data = { items: numberOrChord, type };
        if (typeIsDefault)
            data.typeIsDefault = true;
        if (items[BodyMatch.NOTATION])
            data.notations = items[BodyMatch.NOTATION];
        if (items[BodyMatch.STEP])
            data.step = items[BodyMatch.STEP];
        if (items[BodyMatch.CHORD_SYMBOL])
            data.chordSymbol = items[BodyMatch.CHORD_SYMBOL];
        if (items[BodyMatch.LYRICS])
            data.lyrics = items[BodyMatch.LYRICS];
        if (items[BodyMatch.SOLFA])
            data.solfa = items[BodyMatch.SOLFA];
        if (items[BodyMatch.FUNCTION])
            data.function = items[BodyMatch.FUNCTION];
        if (items[BodyMatch.GRACE])
            data.grace = items[BodyMatch.GRACE];
        if (items[BodyMatch.DOTS])
            data.dots = items[BodyMatch.DOTS].length;
        if (items[BodyMatch.TIE])
            data.tie = true;
        if (numberOrChord.substring(-1) === 'r')
            data.rest = true;
        if (items[BodyMatch.SLUR_START])
            this.addSlurs(data, 'start', items[BodyMatch.SLUR_START].length);
        if (items[BodyMatch.SLUR_END])
            this.addSlurs(data, 'end', items[BodyMatch.SLUR_END].length);
        if (items[BodyMatch.TRIPLETS_P])
            this.addTriplet(data, items[BodyMatch.TRIPLETS_P], items[BodyMatch.TRIPLETS_Q], items[BodyMatch.TRIPLETS_R]);
        return data;
    }
    addSlurs(data, type, count) {
    ...
    addSlur(data, type, index) {
        if (!data.slurs)
            data.slurs = [];
        data.slurs.push({ type, index });
    }
    addTriplet(data, p, q, r) {
        const triplet = { p: parseInt(p) };
        if (q)
            triplet.q = parseInt(q);
        if (r)
            triplet.r = parseInt(r);
        data.triplet = triplet;
    }
    getNoteType(items, inGroup) {
        let noteType = items[BodyMatch.LENGTH_3] || items[BodyMatch.LENGTH_2] || items[BodyMatch.LENGTH] || 'q';
        switch (noteType) {
            case 'd':
                noteType = 'b';
                break;
    ...
    processBodyGroup(group, bar) {
        ...
        if (hasDefaultType)
            this.processBodyGroupDuration(group, bar);
    }
    processBodyGroupBeams(group) {
     ...
     */
    processBodyGroupDuration(group, bar) {
        ...
        const change = new Fraction(group.length)
            ...
            .mul(new Fraction(group.length + (excludeTripletCount > 0 ? tripletNoteCountAdd : 0), group.length - excludeTripletCount + (excludeTripletCount > 0 ? tripletNoteCountAdd : 0)));
        durations.forEach((dur, index) => {
            const data = Duration.getTypeAndDotsFromFraction(dur.mul(change));
            group[index].type = data.type;
            if (data.dots)
                group[index].dots = data.dots;
        });
        return group;
    }
    /*
  ...
  */
    startTriplet(item) {
        if (!item.triplet)
            throw new Error('No triplet data');
    ...
    getTripletDefaultQ(_p) {
        return 2;
    }
    getTripletDefaultR(_p) {
        return 3;
    }
    /**
   ...
   */
            ...
            if (data.rest) {
                ...
                if (!item) {
                    if (pitch[2] === 'y') {
                        ...
                        item = {
                            item: Note.fromScaleNumber((pitch[2] && pitch[2] !== 'i' ? pitch[2] : '1'), (info.octave + noteOctaveShift), info.scale, data.type, data.dots, true),
                        };
                        this.checkTie(data, item.item);
                        this.checkTriplet(data, item);
                        if (data.beam)
                            item.item.beam = { value: data.beam };
                        if (data.slurs)
                            item.item.slurs = data.slurs;
                        if (pitch[2] === 'i' || pitch[3].includes('I'))
                            item.item.invisible = true;
                        if (pitch[3].includes('L'))
                            item.item.locked = true;
                        if (pitch[4]) {
                            const styles = Styles.processStyles(pitch[4]);
                            if (styles.color)
                                item.item.color = { notehead: styles.color };
                            if (styles.size)
                                item.item.size = styles.size;
                        }
                    }
                }
                else {
                    const chordNote = Note.fromScaleNumber((pitch[2] && pitch[2] !== 'i' ? pitch[2] : '1'), (info.octave + noteOctaveShift), info.scale, data.type, data.dots, true);
                    if (item.item.tie)
                        chordNote.tie = item.item.tie;
                    item.item.addChordNote(chordNote);
                    if (pitch[2] === 'i' || pitch[3].includes('I'))
                        item.item.invisible = true;
                    if (pitch[3].includes('L'))
                        chordNote.locked = true;
                    if (pitch[4]) {
                        const styles = Styles.processStyles(pitch[4]);
                        if (styles.color)
                            chordNote.color = { notehead: styles.color };
                        if (styles.size)
                            chordNote.size = styles.size;
                    }
                }
                this.addData(item.item, data);
                if (data.notations)
                    this.processNotation(item.item, data.notations);
                if (data.grace)
                    this.processGrace(item.item, data.grace, info);
    ...
    addData(item, data) {
        if (data.step)
            item.analysis = { romanNumeral: [{ step: data.step }] };
        this.setChordSymbol(item, data.chordSymbol);
        if (data.lyrics)
            item.lyrics = [{ text: data.lyrics }];
        if (data.solfa)
            item.solfege = data.solfa;
        if (data.function) {
            if (!item.analysis) {
                item.analysis = {};
            }
            item.analysis.function = [{ function: data.function }];
        }
    }
    setChordSymbol(note, chordSymbol, offset) {
        if (offset) {
            this.errors.push('Chord offset not implemented');
            return;
        }
        if (chordSymbol)
            note.setChordSymbol(new ChordSymbol(chordSymbol));
    }
    processNotation(note, item) {
        if (!item)
            return;
        let articulation;
        ...
        items.forEach((item) => {
            ...
            if (item === 'fermata' || item === 'invertedfermata') {
                ...
                item === 'invertedturn' ||
                item === 'invertedturnx') {
                notation = new Notations.Turn(item === 'invertedturn' || item === 'invertedturnx', item === 'turnx' || item === 'invertedturnx');
            }
            else if (item === 'arpeggio') {
                notation = new Notations.Arpeggio();
            }
            else if ((articulation = articulations.find((a) => a[0] === item))) {
                notation = new Notations.Articulation(articulation[1]);
            }
                ...
                notation = this.processNotationDuration(Notations.Trill, 'trill', item);
            }
            else if (item.slice(0, 5) === 'cresc') {
                notation = this.processNotationDuration(Notations.Crescendo, 'crescendo', item);
            }
            else if (item.slice(0, 3) === 'dim') {
                notation = this.processNotationDuration(Notations.Diminuendo, 'diminuendo', item);
            }
            else if (item === 'editorial') {
                if (!note.printedAccidental)
                    note.printedAccidental = { value: note.accidental };
    ...
    processNotationDuration(type, name, item) {
    ...
    processGrace(note, graceData, info) {
