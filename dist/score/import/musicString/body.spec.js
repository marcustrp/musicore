...
import { BodyMatch, BodyParser } from './body.js';
...
describe('parse()', () => {
    it('should parse rest', () => {
        ...
        const result = parser.parse(musicstring, info, new Bar(new TimeSignature(), new Key('c', 'major')));
        expectedResult[0].item['id'] = result[0].item.id;
        expect(result).toStrictEqual(expectedResult);
    });
    it('should parse number with accidental', () => {
        const musicstring = '#4';
        const expectedResult = [{ item: { name: 'f#' } }];
        const result = parser.parse(musicstring, info, new Bar(new TimeSignature(), new Key('c', 'major')));
    ...
    describe('groups', () => {
        it('should return correct octave for 321-7', () => {
            ...
            const note1 = { root: 'e', octave: 5 };
            const note2 = { root: 'd', octave: 5 };
            const note3 = { root: 'c', octave: 5 };
            const note4 = { root: 'b', octave: 4 };
        ...
        it('should return correct octave for 17+23', () => {
            ...
            const note1 = { type: '16', root: 'c', octave: 5 };
            const note2 = { type: '16', root: 'b', octave: 5 };
            const note3 = { type: '16', root: 'd', octave: 6 };
            const note4 = { type: '16', root: 'e', octave: 6 };
            const expectedResult = [{ item: note1 }, { item: note2 }, { item: note3 }, { item: note4 }];
            const result = parser.parse(musicstring, info, new Bar(new TimeSignature(), new Key('c', 'major')));
        ...
        it('should return correct octave for 1_/51', () => {
            ...
            const note1 = { type: '8', root: 'c', octave: 5, beam: { value: 'start' } };
            const note2 = { type: '16', root: 'g', octave: 4, beam: { value: 'continue' } };
            const note3 = { type: '16', root: 'c', octave: 5, beam: { value: 'end' } };
    ...
    it('should parse note with two grace notes', () => {
        ...
        const result = parser.parse(musicstring, info, new Bar(new TimeSignature(), new Key('c', 'major')));
        expect(result[0].item.graceNotes).toHaveLength(2);
        expect(result).toMatchObject([
            {
                item: {
                    _root: 'e',
...
describe('process()', () => {
    it('should return a note', () => {
        ...
        const note = Note.fromScaleNumber(data.items, 5, scale, data.type);
  ...
  it('should return a note with chord notes, where all notes are tied', () => {
    ...
    it('should return a note with step (analysis)', () => {
        ...
        const note = Note.fromScaleNumber(data.items, 5, scale, data.type);
        note.analysis = { romanNumeral: [{ step: 'IVm' }] };
        const expectedResult = { item: note };
        const result = parser.process(data, info);
        note['id'] = result.item.id;
        expect(result).toEqual(expectedResult);
    ...
    it('should return a note/rest with chord symbol', () => {
        const data = [
            { items: '2', type: 'h', chordSymbol: 'Eb' },
            { items: '1', rest: true, type: 'h', chordSymbol: 'F#' },
        ];
        const note = Note.fromScaleNumber(data[0].items, 5, scale, data[0].type);
        note['_chordSymbols'] = [{ text: 'Eb' }];
        const expectedResult = { item: note };
        const result = parser.process(data[0], info);
        const note2 = new Rest(data[1].type);
        note2['_chordSymbols'] = [{ text: 'F#' }];
    ...
    it('should return a note with function (analysis)', () => {
        ...
        const note = Note.fromScaleNumber(data.items, 5, scale, data.type);
        note.analysis = { function: [{ function: 'T5' }] };
        const expectedResult = { item: note };
        const result = parser.process(data, info);
        note['id'] = result.item.id;
        expect(result).toEqual(expectedResult);
