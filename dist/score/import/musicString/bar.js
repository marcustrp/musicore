import {} from '../../../core/bar.js';
import { Coda, DaCapo, Fine, Segno } from '../../../core/data/directions.js';
import { Fermata } from '../../../core/data/notations.js';
var BarMatch;
(function (BarMatch) {
    BarMatch[BarMatch["DIRECTIONS"] = 1] = "DIRECTIONS";
    BarMatch[BarMatch["MATCH1"] = 2] = "MATCH1";
    BarMatch[BarMatch["MATCH2"] = 3] = "MATCH2";
})(BarMatch || (BarMatch = {}));
export class BarlineParser {
    ...
    Identify musicstring bar item
        */
        ...
        if (items[BarMatch.DIRECTIONS])
            data.directions = items[BarMatch.DIRECTIONS];
        return data;
    }
    process(item) {
                ...
                if (!state.inEnding) {
                    switch (char) {
                        ...
                        case '9':
                            ...
                            else if ((data.barline === 'regular' || data.barline === 'light-heavy') &&
                                !state.dot &&
                                !state.startBracket) {
                                data.barline = 'light-heavy';
                                state.inEnding = true;
    ...
    getDaCapo(item) {
        ...
        if (item.length > 10)
            dc.extra = item.slice(10);
        return dc;
    }
    addError(index, char, data, tokenType = 'barline') {
