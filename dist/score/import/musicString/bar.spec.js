...
import { BarlineParser } from './bar.js';
let parser;
let errors;
beforeEach(() => {
    errors = [];
...
describe('process()', () => {
    describe('barlines', () => {
        it('should process single barline', () => {
            const expectedResult = { barline: 'regular' };
        ...
        it('should process double barline', () => {
            const expectedResult = { barline: 'light-light' };
            const result = parser.process({ data: '||' });
            if (errors.length > 0)
                errors.forEach((item) => console.log('####' + item));
            expect(errors.length).toBe(0);
        ...
        it('should process barline ending |]', () => {
            const expectedResult = { barline: 'light-heavy' };
            const result = parser.process({ data: '|]' });
            if (errors.length > 0)
                errors.forEach((item) => console.log('####' + item));
            expect(errors.length).toBe(0);
        ...
        it('should process barline "start" [|', () => {
            const expectedResult = { barline: 'heavy-light' };
            const result = parser.process({ data: '[|' });
            if (errors.length > 0)
                errors.forEach((item) => console.log('####' + item));
            expect(errors.length).toBe(0);
        ...
        it('should process hidden barline [|]', () => {
            const expectedResult = { barline: 'none' };
            const result = parser.process({ data: '[|]' });
            if (errors.length > 0)
                errors.forEach((item) => console.log('####' + item));
            expect(errors.length).toBe(0);
    ...
    describe('repeats', () => {
        describe('repeat start', () => {
            const data = ['|:', '||:', '[|:'];
            const expectedResult = { barline: 'heavy-light', repeatStart: 1 };
            for (let i = 0; i < data.length; i++) {
        ...
        describe('repeat end', () => {
            const data = [':|', ':||', ':|]'];
            const expectedResult = { barline: 'light-heavy', repeatEnd: 1 };
            for (let i = 0; i < data.length; i++) {
        ...
        describe('repeat start and end', () => {
            const data = ['::', ':|:', ':|:', ':||:', ':|][|:'];
            const expectedResult = { barline: 'heavy-heavy', repeatEnd: 1, repeatStart: 1 };
            for (let i = 0; i < data.length; i++) {
    ...
    describe('endings', () => {
        ...
        it('should process first ending |[1', () => {
            const expectedResult = { barline: 'regular', ending: '1' };
            const result = parser.process({ data: '|[1' });
            if (errors.length > 0)
                errors.forEach((item) => console.log('####' + item));
            expect(errors.length).toBe(0);
        ...
        it('should process second ending |2', () => {
            const expectedResult = { barline: 'light-heavy', ending: '2' };
            const result = parser.process({ data: '|2' });
            if (errors.length > 0)
                errors.forEach((item) => console.log('####' + item));
            expect(errors.length).toBe(0);
        ...
        it('should process second ending with repeat :|2', () => {
            const expectedResult = { barline: 'light-heavy', ending: '2', repeatEnd: 1 };
            const result = parser.process({ data: ':|2' });
            if (errors.length > 0)
                errors.forEach((item) => console.log('####' + item));
            expect(errors.length).toBe(0);
        ...
        it('should process ending [1,2', () => {
            const expectedResult = { ending: '1,2' };
        ...
        it('should process ending :|][1-2', () => {
            const expectedResult = { barline: 'light-heavy', repeatEnd: 1, ending: '1-2' };
            const result = parser.process({ data: ':|][1-2' });
            if (errors.length > 0)
                errors.forEach((item) => console.log('####' + item));
            expect(errors.length).toBe(0);
    ...
    describe('line breaks', () => {
        it('should process just a line break', () => {
            const expectedResult = { lineBreak: true };
            const result = parser.process({ data: '\\' });
            if (errors.length > 0)
                errors.forEach((item) => console.log('####' + item));
            expect(errors.length).toBe(0);
...
describe('processData()', () => {
    it('should process ||\\', () => {
        const data = '||\\';
        const expecedResult = { barline: 'light-light', lineBreak: true };
    ...
    it('should process |:\\', () => {
        const data = '|:\\';
        const expecedResult = { barline: 'heavy-light', lineBreak: true, repeatStart: 1 };
        const result = parser.processData(data);
        expect(errors.length).toBe(0);
        expect(result).toStrictEqual(expecedResult);
    });
