...
import {} from '../core/data/directions.js';
...
export class BarArray {
    ...
    constructor(timeSignature, key) {
        this.bars = [new Bar(timeSignature, key, true)];
        this.duration = new Fraction(timeSignature.duration);
    }
    appendBar(timeSignature, key, showKeySign = false) {
        if (!timeSignature)
            timeSignature = this.bars[this.bars.length - 1].timeSignature;
        if (!key)
    ...
    removeBars(indexFrom, indexTo) {
        const deleteCount = indexTo === undefined ? this.bars.length - indexFrom : indexTo - indexFrom + 1;
     ...
     * @param indexTo
     */
    setKey(key, indexFrom = 0, indexTo) {
        if (indexFrom < 0)
            indexFrom = 0;
        if (indexTo === undefined || indexTo >= this.bars.length)
            indexTo = this.bars.length - 1;
     ...
     */
    addNote(partId, voiceId, note, bar, beat, options) {
        if (bar !== undefined || beat !== undefined)
     ...
     */
    addNotes(partId, voiceId, notes, bar, beat, ignoreOverflow = false) {
        if (bar !== undefined || beat !== undefined)
            throw new Error('Not implemented');
        notes.forEach((note) => this.addNote(partId, voiceId, note, bar, beat, {
            overflow: ignoreOverflow ? 'ignore' : 'split',
        }));
    }
    addLineBreak(barIndex) {
        if (barIndex === undefined)
            barIndex = this.bars.length - 1;
        this.bars[barIndex].lineBreak = true;
    }
    getBarNotesDuration(partId, voiceId, bar) {
        if (!bar)
            throw new Error('Bar is undefined');
        return bar.getVoiceDuration(partId, voiceId);
    }
    getNote(partId, voiceId, barIndex, noteIndex) {
        if (!partId || !voiceId)
            throw new Error('Voice not found');
        if (barIndex === undefined)
            barIndex = this.getLastBarWithVoice(partId, voiceId);
        if (barIndex === undefined || barIndex < 0 || barIndex >= this.bars.length)
            throw new Error('Bar index out of range');
        if (noteIndex === undefined)
            return this.getLastNoteInBar(barIndex, partId, voiceId);
        if (noteIndex === undefined)
    ...
    getNoteByIndex(partId, voiceId, noteIndex) {
        if (!partId || !voiceId)
            throw new Error('Voice not found');
        let index = 0;
        let barIndex = 0;
    ...
    getNoteById(partId, voiceId, noteId) {
        if (!partId || !voiceId)
            throw new Error('Voice not found');
    ...
    getNextNote(partId, voiceId, barIndex, noteIndex) {
        ...
        else if (barIndex + 1 < this.bars.length &&
    ...
    getNotes(partId, voiceId, barIndex) {
    ...
    getLastBarWithVoice(partId, voiceId) {
        for (let i = this.bars.length - 1; i >= 0; i--) {
            if (partId in this.bars[i].notes) {
                if (voiceId in this.bars[i].notes[partId])
                    return i;
            }
        }
        return undefined;
    }
    getLastNoteInBar(barIndex, partId, voiceId) {
        if (barIndex < 0 || barIndex >= this.bars.length)
            throw new Error('Bar index out of range');
        if (!(partId in this.bars[barIndex].notes && voiceId in this.bars[barIndex].notes[partId]))
            return undefined;
        return this.bars[barIndex].notes[partId][voiceId][this.bars[barIndex].notes[partId][voiceId].length - 1];
    }
    /**
     ...
     * @param barIndex If undefined, last bar is selected
     */
    setRepeatStart(repeat, barIndex) {
        if (barIndex === undefined)
            barIndex = this.bars.length - 1;
     ...
     * @param barIndex If undefined, last bar is selected
     */
    addDirection(item, barIndex) {
        if (barIndex === undefined)
            barIndex = this.bars.length - 1;
        if (barIndex < 0 || barIndex >= this.bars.length)
            throw new Error('BarIndex out of bounds');
        this.bars[barIndex].addDirection(item);
    }
    setTempo(value, barIndex = 0, position) {
        if (barIndex >= this.bars.length)
            throw new Error('Bar index out of bounds');
