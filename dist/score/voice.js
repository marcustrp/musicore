...
export class Voice {
     ...
     */
    addNote(note, bar, beat, ignoreOverflow = false) {
     ...
     * @param ignoreOverflow
     */
    addNotes(notes, bar, beat, ignoreOverflow = false) {
     ...
     */
    addTriplet(notes, numerator, denominator, bar, beat) {
     ...
     * @param noteIndex
     */
    getNextNote(barIndex, noteIndex) {
     ...
     * @todo unit tests
     */
    addTie(barIndex, noteIndex) {
        const note = this.getNote(barIndex, noteIndex);
        if (!(note instanceof Note))
            throw new Error('Tie can only be set on notes');
        if (note.hasTie(true))
            return false;
        const nextNote = this.getNextNote(barIndex, noteIndex);
        if (!(nextNote instanceof Note))
            return false;
        note.tie = note.tie === 'end' ? 'continue' : 'start';
        nextNote.tie = nextNote.tie === 'start' ? 'continue' : 'end';
    }
    /**
     *
     * @param barIndex
