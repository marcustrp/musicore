 ...
 */
export const validateScore = (score, addWarning, addError) => {
    if (score.bars.bars.length === 0)
        throw new Error('No bars in score');
    let previousTimeSignature;
    if (score.bars.bars.filter((bar) => {
        let diff = false;
        if (!bar.pickup &&
    ...
    if (score.parts.getPartCount() === 0)
        addError('No part in score');
    if (score.parts.getPartCount() > 1)
        addWarning('Only first part will be exported');
    if (score.parts.getPart(0).getStaffCount() === 0)
        addError('No stave in score');
    if (score.parts.getPart(0).getStaffCount() > 1)
        addWarning('Only first stave will be exported');
    if (score.parts.getPart(0).getVoiceCount() > 1)
        addWarning('Only first voice will be exported');
