...
export class NoteGenerator {
    ...
    getNote(note, scaleNotes, timeSignature) {
        let abc = this.getNoteBeam(note);
        if (note.slurs)
            abc += this.getSlurs('start', note.slurs);
        ...
        if (note.tie === 'start')
            abc += '-';
        if (note.slurs)
            abc += this.getSlurs('end', note.slurs);
        if (note.size && note.size !== 'normal')
            console.warn('Note size not supported in ABC');
        return abc;
    }
    getRest(rest) {
        let abc = 'z';
    ...
    getTriplet(note, timeSignature) {
        ...
        if (note.triplet.start) {
            ...
            const defaultDenomintor = this.getTripletDefaultDenominator(note.triplet.numerator, timeSignature);
            if (note.triplet.noteCount !== note.triplet.numerator &&
            ...
            else if (note.triplet.noteCount !== note.triplet.numerator &&
     ...
     */
    getTripletDefaultDenominator(numerator, timeSignature) {
        switch (numerator) {
            case 2:
            case 4:
            case 8:
    ...
    getNoteDecorations(note) {
        ...
        note.notations.forEach((notation) => {
            ...
            else if (notation instanceof Notations.Harmonic ||
        ...
        if (note.printedAccidental) {
            if (note.printedAccidental.editorial)
                decorations += 'editorial';
            if (note.printedAccidental.parentheses)
                decorations += 'courtesy';
        }
        return decorations;
    }
    getChordSymbols(note) {
        let abc = '';
    ...
    getNoteAccidental(note, scaleNotes) {
    ...
    getNoteArticulation(articulation) {
     ...
     */
    getNoteLength(note, defaultLength) {
        ...
        if (abc.charAt(abc.length - 1) === '/')
            return abc.slice(0, abc.length - 1);
        return abc;
    }
}
