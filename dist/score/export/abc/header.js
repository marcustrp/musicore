...
export class HeaderGenerator {
    addWarning;
    validateError;
    constructor(addWarning, validateError) {
...
X:1`;
        if (!settings || settings.header !== 'minimal') {
            ...
            if (work) {
                if (work.title)
                    header += this.getString('T', work.title);
                if (work.subtitle)
                    header += this.getString('T', work.subtitle);
                if (work.creator)
                    header += this.getComposers(work.creator);
            }
            const information = score.information;
            if (information) {
                if (information.books)
                    header += this.getArray('B', information.books);
                if (information.discography)
                    header += this.getArray('D', information.discography);
                if (information.notes)
                    header += this.getString('N', information.notes);
                if (information.origin)
                    header += this.getString('O', information.origin);
                if (information.type)
                    header += this.getString('R', information.type);
                if (information.source)
                    header += this.getString('S', information.source);
...
L:${abcLengthUnit}`;
     ...
     * @returns
     */
    getTimeSignature(score) {
        const timeSignature = score.bars.bars[0].pickup ?
    ...
    getTempo(bar) {
        if (bar.tempo)
            return this.getString('Q', bar.tempo[0].value);
        return '';
    }
    /**
    ...
    getArray(field, data) {
        if (!data)
            throw new Error('getArray: data empty');
        let abc = '';
     ...
     */
    getComposers(creators) {
        let abc = '';
        creators.forEach((creator) => {
            if (creator.type === 'composer')
                abc += this.getString('C', creator.text);
        });
        return abc;
    }
}
