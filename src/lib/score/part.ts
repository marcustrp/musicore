...
*/
  ...
  private getStaff(staffIndex: number) {
    if (this.staves.length <= staffIndex || staffIndex < 0) throw new Error('Staff index out of range');
  ...
  getVoice(voiceIndex: number) {
    if (this.voices.length <= voiceIndex || voiceIndex < 0) throw new Error('Voice index out of range');
