...
export class MetaTextParser {
  ...
  parse(metaText: abcjs.MetaText, score: Score) {
    ...
    if (metaText['abc-copyright']) score.addInformation('transcription.copyright', metaText['abc-copyright']);
    if (metaText['abc-creator']) score.addInformation('transcription.creator', metaText['abc-creator']);
    if (metaText['abc-edited-by']) score.addInformation('transcription.editedBy', metaText['abc-edited-by']);
    ...
    if (metaText.partOrder) this.state.errors.push('metaText.partOrder not supported: ' + metaText.partOrder); //score.addInformation(, metaText.partOrder);
