...
export class NoteParser {
  ...
  parse(item: abcjsTypes.VoiceItemNote_FIX, score: Score) {
    ...
    if (item.chord) item.chord.forEach((chord) => note.setChordSymbol(this.createChordSymbol(chord.name)));
    if (item.startBeam) {
  ...
  private tripletContinue(item: abcjs.VoiceItemNote, note: Note) {
    ...
    if (item.endTriplet) {
      if (this.tripletState.currentCount !== this.tripletState.noteCount) throw new Error('Triplet not count error');
      return true;
    }
    return false;
  }
