...
export class InformationParser {
  ...
  parse(input: string, index: number, currentInfo: InformationItem) {
    ...
    while (input) {
      ...
      let data: InformationData<string | number | TimeSignature | Clef | KeyData | Scale> | undefined;
      ...
      switch (type) {
        ...
        case 'E':
          ...
          break;
        case 'I':
          const root = currentInfo.key ? currentInfo.key.root : item.key ? item.key.root : 'c';
  ...
  private getKey(input: string) {
    ...
    if (match && match.length > 0) {
      let keyData = this.getMode(match[3] ? match[3] : /[A-Z]/.test(match[1]) ? 'ma' : 'mi');
  ...
  private getTimeSignatureObject(length: number, count: number, unit: number, symbol?: TimeSignatureSymbol) {
    const data: InformationData<TimeSignature> = { length, data: new TimeSignature(symbol ? symbol : count, unit) };
    return data;
  }
  ...
  private getClef(input: string) {
    ...
    if (match) {
      if (match[1] === 'p' || match[1] === 'n') {
        ...
        return this.getClefObject(match[0].length, clefType[match[1] as ClefTypes], undefined, undefined, staffLines);
      } else if (match[1]) {
        ...
        return this.getClefObject(match[0].length, clefType[match[1] as ClefTypes], line, octaveChange);
      }
    }
    this.errors.push(`Clef: empty or invalid data: ${input}`);
  }

  getClefObject(length: number, type: ClefType, line?: number, octaveChange?: 2 | 1 | -1 | -2, staffLines?: number) {
  ...
  setDefaults(header: InformationItem) {
    ...
    const item: InformationItem = { ...header, clef, scale, inputScale, key, timeSignature, octave };
    return item;
  }
}
