...
describe('getClef()', () => {
  it('should return correct clef', () => {
    ...
    const expecedResult: InformationData<Clef> = { length: musicstring.length, data: new Clef('f') };
...
describe('getKey()', () => {
  it('should return correct data', () => {
    ...
    const expectedResult: InformationData<KeyData> = {
      length: 5,
      data: { key: new Key('eb', 'lydian'), scale: new Scale('eb', 'lydian'), inputScale: new Scale('eb', 'lydian') },
    };
  ...
  it('should return correct data (short mode, minor)', () => {
    ...
    const expectedResult: InformationData<KeyData> = {
      length: musicstring.length,
      data: { key: new Key('d', 'minor'), scale: new Scale('d', 'minor'), inputScale: new Scale('d', 'minor') },
    };
    const result = informationParser['getKey'](musicstring);
    expect(result).toStrictEqual(expectedResult);
  });
  ...
  it('should return correct default header', () => {
    ...
    const defaultHeader: InformationItem = {
      ...
      timeSignature: {
        ...
        beamGroupDurations: [new Fraction(1, 4), new Fraction(1, 4), new Fraction(1, 4), new Fraction(1, 4)],
...
describe('getTimeSignature()', () => {
  ...
  it('should correctly parse single number', () => {
    ...
    const expecedResult: InformationData<TimeSignature> = { length: musicstring.length, data: new TimeSignature(3, 4) };
    const result = informationParser['getTimeSignature'](musicstring);
    expect(errors.length).toBe(0);
    expect(result).toStrictEqual(expecedResult);
  });
  it('should correctly parse double number', () => {
    ...
    const expecedResult: InformationData<TimeSignature> = { length: musicstring.length, data: new TimeSignature(6, 8) };
    const result = informationParser['getTimeSignature'](musicstring);
    expect(errors.length).toBe(0);
    expect(result).toStrictEqual(expecedResult);
  });
