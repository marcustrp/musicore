...
export class BarlineParser {
  ...
  match(item: string) {
    ...
    const data: BarData = { data: items[BarMatch.MATCH1] ? items[BarMatch.MATCH1] : items[BarMatch.MATCH2] };
  ...
  process(item: BarData) {
    ...
    if (item.directions) {
      ...
      if (directions && directions.current && directions.current.length) data.directionsCurrent = directions.current;
      if (directions && directions.next && directions.next.length) data.directionsNext = directions.next;
    }
    return data;
  }
    ...
    if (item === '::') {
      ...
      for (let i = 0; i < item.length; i++) {
        ...
        if (!state.inEnding) {
          ...
          if (+char >= 1 && +char <= 9 && state.endingNumber === 0 && (state.endingComma || state.endingHypen)) {
            state.endingNumber = +char;
