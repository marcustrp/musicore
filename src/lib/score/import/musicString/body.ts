...
export class BodyParser {
  ...
  matchParse(items: string[], inGroup = false) {
    ...
    if (items[BodyMatch.SLUR_START]) this.addSlurs(data, 'start', items[BodyMatch.SLUR_START].length);
  ...
  */
  ...
  addData(item: Note | Spacer, data: BodyData) {
    if (data.step) item.analysis = { romanNumeral: [{ step: data.step } as any as RomanNumeralAnalysis] };
  ...
  processNotation(note: Note | Spacer, item: string) {
    ...
    items.forEach((item) => {
      ...
      if (item === 'fermata' || item === 'invertedfermata') {
      ...
      } else if (item === 'turn' || item === 'turnx' || item === 'invertedturn' || item === 'invertedturnx') {
        ...
        notation = this.processNotationDuration<Notations.Crescendo>(Notations.Crescendo, 'crescendo', item);
      } else if (item.slice(0, 3) === 'dim') {
        notation = this.processNotationDuration<Notations.Diminuendo>(Notations.Diminuendo, 'diminuendo', item);
