...
export class MusicStringImporter {
  ...
  parse(musicString: string) {
    ...
    items.forEach((item, index) => {
      ...
      if (!this.score) {
        ...
        if ((!octaveSet && info?.clef?.type === 'f') || info?.clef?.type === 'bass') this.info.octave = 3;
  ...
  createScore(info: InformationItem) {
    ...
    if (info.discography) info.discography.forEach((disc) => this.score.addInformation('discography', disc));
    ...
    if (info.composers) info.composers.forEach((composer) => this.score.addWorkInformation('composer', composer));
  }
  ...
  handleBarItem(item: BarItem) {
    ...
    if (item.repeatEnd && firstBarVoiceDuration.compare(0) != 0) this.score.bars.setRepeatEnd(item.repeatEnd);
