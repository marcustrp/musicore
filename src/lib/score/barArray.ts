...
export class BarArray {
  ...
  appendBar(timeSignature?: TimeSignature, key?: Key, showKeySign = false) {
    ...
    if (!key) key = this.bars.length > 0 ? this.bars[this.bars.length - 1].key : new Key('c', 'major');
   ...
   */
  addNote(partId: string, voiceId: string, note: Note | Rest, bar?: number, beat?: Fraction, options?: AddNoteOptions) {
    if (bar !== undefined || beat !== undefined) throw new Error('Not implemented');

    if (
      options &&
      (options.overflow === 'ignore' ||
        this.last.duration.sub(this.last.getVoiceDuration(partId, voiceId)).compare(note.getDuration()) >= 0)
    ) {
      ...
      if (this.last.duration.compare(this.last.getVoiceDuration(partId, voiceId)) <= 0) this.appendBar();
  ...
  addNotes(
    ...
    if (bar !== undefined || beat !== undefined) throw new Error('Not implemented');
    notes.forEach((note) =>
      this.addNote(partId, voiceId, note, bar, beat, { overflow: ignoreOverflow ? 'ignore' : 'split' }),
    );
  }
  ...
  getNotes(partId: string, voiceId: string, barIndex?: number) {
    ...
    if (barIndex === undefined) {
      ...
      if (!(partId in this.bars[barIndex].notes && voiceId in this.bars[barIndex].notes[partId])) return [];
  ...
  private getLastNoteInBar(barIndex: number, partId: string, voiceId: string) {
    ...
    if (!(partId in this.bars[barIndex].notes && voiceId in this.bars[barIndex].notes[partId])) return undefined;
