...
export class NoteGenerator {
  ...
  private getSlurs(type: 'start' | 'end', slurs: { type: 'start' | 'end'; index?: number }[]) {
    ...
    if (slurs && slurs.length > 0) {
      slurs.forEach((slur) => (abc += slur.type === type ? (type === 'start' ? '(' : ')') : ''));
    }
    return abc;
  }
  ...
  private getNoteDecorations(note: Note) {
    ...
    note.notations.forEach((notation) => {
      if (notation instanceof Notations.Fermata) {
        ...
        decorations += '!slide!';
      } else if (notation instanceof Notations.Harmonic || notation instanceof Notations.OpenString) {
        decorations += '!open!';
  ...
  private getAnalysis(note: Note) {
    ...
    if (note.analysis) {
      if (note.analysis.romanNumeral) {
        if (note.analysis.romanNumeral.length > 1) throw new Error('Multiple roman numerals not supported');
        abc += `"^${note.analysis.romanNumeral[0].step}"`;
      }
      if (note.analysis.function) {
        if (note.analysis.function.length > 1) throw new Error('Multiple roman numerals not supported');
  ...
  private getNoteAccidental(note: Note, scaleNotes: string[]) {
    ...
    return note.name.slice(1).replace('bb', '__').replace('b', '_').replace('#', '^').replace('x', '^^');
  }
