...
export class PitchStreamExporter {
    ...
    barIndexSequence.forEach((barIndex) => {
      ...
      notes.forEach((note) => {
        if (this.includeNote(note, settings))
          pitchStream.push(scale.getScaleNumberFromNote((note as Note).root, (note as Note).accidental));
      });
    });
    return pitchStream;
  }
  ...
  getScaleNumberChangeClosest(score: Score, barIndexSequence: number[], settings: ExportSettings) {
    ...
    let previous = -1;
    do {
      let note = score.parts.getPart(settings.part).getVoice(settings.voice).getNoteByIndex(firstIndex);
      if (note && this.includeNote(note, settings)) previous = nameToNumber[(note as Note).name.charAt(0)];
      firstIndex++;
      if (firstIndex > 10) {
    ...
    barIndexSequence.forEach((barIndex) => {
      ...
      notes.forEach((note) => {
        if (firstIndex === 0 && this.includeNote(note, settings)) {
          ...
          if (
            diff === 0 &&
            (settings.type === 'diffClosest-ignoreRepeating' || settings.type === 'diffClosestPositive-ignoreRepeating')
          )
            return;

          let change = diff < -3 ? diff + 7 : diff > 3 ? diff - 7 : diff;
          if (settings.type === 'diffClosestPositive' || settings.type === 'diffClosestPositive-ignoreRepeating')
            change += 3;
