...
export class Voice {
  ...
  addNote(note: Note | Rest, bar?: number, beat?: Fraction, ignoreOverflow = false) {
    this.bars.addNote(this.partId, this.id, note, bar, beat, { overflow: ignoreOverflow ? 'ignore' : 'split' });
  }
   ...
   */
  addTriplet(notes: (Note | Rest)[], numerator: number, denominator: number, bar?: number, beat?: Fraction) {
