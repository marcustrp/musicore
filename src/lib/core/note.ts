...
export class Note extends RhythmElement {
  ...
  static fromScaleNumber(
    ...
    scaleNumber = ((processedParts.accidental || '') + processedParts.number.toString()) as ScaleNumber;
   ...
   * @returns
   */
  private static processScaleNumber(scaleNumberParts: ScaleNumberParts, scale: Scale, useScaleMode: boolean) {
  ...
  removePrintedAccidental() {
    ...
    if (this.name !== this.diatonicNoteName) {
      this.setAccidental(
        this.diatonicNoteName.length === 1 ? undefined : (this.diatonicNoteName.substr(1) as NoteAccidentals),
      );
    }
  }
  ...
  static getNumberParts(noteNumber: string) {
    ...
    if (noteNumberParts[1].length > 0) parts.accidental = noteNumberParts[1] as ScaleNumberAccidentals;
    return parts;
  }
