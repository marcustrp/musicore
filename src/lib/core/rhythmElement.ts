...
export class RhythmElement {
  ...
  private findChordSymbol(chordSymbol: ChordSymbol, offset: Fraction | undefined, level: number | undefined) {
    if (
      offset &&
      ...
      return false;
    if (level && chordSymbol.level !== level && !(level === undefined && chordSymbol.level === undefined)) return false;
  ...
  getDuration(ignoreTriplet = false) {
    const fraction =
      this.triplet && !ignoreTriplet ? new Fraction(this.triplet.denominator, this.triplet.numerator) : new Fraction(1);
   ...
   */
  setTriplet(numerator: number, denominator: number, totalDuration: Fraction, noteIndex: number, noteCount: number) {
    this.triplet = {
      numerator: numerator,
