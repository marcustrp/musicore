...
describe('setBeamGroupDuration()', () => {
  let spy: SpyInstance;
  const initSpy = () =>
    vi.spyOn(TimeSignature.prototype as any, 'fillBeamGroupDurations').mockImplementationOnce((durations) => {
      return durations;
    });
  ...
  it('should set beamGroupDurations for supported time signatures', () => {
    ...
    expect(spy).toBeCalledWith([new Fraction(3, 8), new Fraction(3, 8), new Fraction(3, 8), new Fraction(3, 8)]);
...
describe('TimeSignature', () => {
  let spy: SpyInstance;
  const initSpy = () =>
    vi.spyOn(TimeSignature.prototype as any, 'setBeamGroupDuration').mockImplementationOnce((durations) => {
      return durations;
    });
