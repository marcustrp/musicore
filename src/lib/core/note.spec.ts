...
describe('Note.fromScaleNumber', () => {
  ...
  it('should create a note using minor scale with m (major) accidental', () => {
    ...
    const spy1 = vi.spyOn(Note, 'nameToNoteIndex').mockImplementationOnce(() => noteIndex);
    const spy2 = vi.spyOn(Note, 'getNumberParts').mockImplementationOnce(() => noteNumberParts);
    const spy3 = vi.spyOn(Note, 'noteNumberToNameInScale').mockImplementationOnce(() => noteObj._root + (noteObj._accidental || ''));
    const result = Note.fromScaleNumber(
      ((noteNumberParts.accidental || '') + noteNumberParts.number) as ScaleNumber,
      5,
      scaleMock.get(scaleData),
  ...
  it('should throw on invalid input', () => {
    const resultFn = () => Note.fromScaleNumber('none' as ScaleNumber, 5, new Scale('c', 'major'), 'h');
