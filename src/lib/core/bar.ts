...
export class Bar {
  ...
  setPickup(duration?: Fraction) {
    ...
    if (duration.valueOf() === 0) throw new Error('Duration is not specified and all voices are empty');
      ...
      Object.entries(parts).forEach(([voiceId, notes]) => {
        notes.forEach((rhythmElement) => {
          ...
          if (item !== note.name) {
            ...
            note.printedAccidental = { value: newAccidental === '' ? 'n' : (newAccidental as Accidentals) };
