...
export class TimeSignature {
   ...
   */
  getNotesFromBeamGroup(startDuration: Fraction, note: Note | Rest, overflow: 'process' | 'return' = 'process') {
    ...
    while (
      ...
      if (groupDurationLeft.compare(noteDurationLeft) > 0) {
        ...
        groupDurationLeft.compare(this.beamGroupDurations[beamGroupIndex % this.beamGroupDurations.length]) === 0 &&
        this.beamGroupIndexesInSameBar(beamGroupIndex, beamGroupIndex + 1) &&
        noteDurationLeft.compare(
          groupDurationLeft.add(this.beamGroupDurations[(beamGroupIndex + 1) % this.beamGroupDurations.length]),
        ) >= 0
      ) {
        console.log(
        ...
        let span = 2;
        beamGroupIndex++;
        let tmpGroupDuration = this.beamGroupDurations[beamGroupIndex % this.beamGroupDurations.length].add(
        ...
        console.log(
          ...
          groupDurationLeft.compare(this.beamGroupDurations[beamGroupIndex % this.beamGroupDurations.length]),
          this.beamGroupIndexesInSameBar(beamGroupIndex, beamGroupIndex + 1),
          beamGroupIndex,
          noteDurationLeft.compare(
            groupDurationLeft.add(this.beamGroupDurations[(beamGroupIndex + 1) % this.beamGroupDurations.length]),
    ...
    if (data.notes.length > 1 && data.notes[0] instanceof Note) {
      for (let i = 0; i < data.notes.length; i++) {
        (data.notes[i] as Note).tie = i === 0 ? 'start' : i < data.notes.length - 1 ? 'continue' : 'end';
      }
    }
    if (data.overflow.length > 1 && data.overflow[0] instanceof Note) {
      for (let i = 0; i < data.overflow.length; i++) {
        (data.overflow[i] as Note).tie = i === 0 ? 'start' : i < data.overflow.length - 1 ? 'continue' : 'end';
      }
    }
    if (overflow === 'process' && data.overflow.length > 0 && data.notes[0] instanceof Note) {
      (data.notes[data.notes.length - 1] as Note).tie = data.notes.length > 1 ? 'continue' : 'start';
  ...
  getBeamGroupPosition(position: Fraction) {
    ...
    return { index: beamGroupIndex, position: currentBeamDuration.sub(this.beamGroupDurations[beamGroupIndex]) };
  }

  private beamGroupIndexesInSameBar(index1: number, index2: number) {
    return Math.floor(index1 / this.beamGroupDurations.length) === Math.floor(index2 / this.beamGroupDurations.length);
  }
